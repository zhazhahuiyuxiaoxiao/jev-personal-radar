...
Jev Personal Radar 的意思是“Jev 个人信息雷达”。每天先从 GitHub 今日/本周热门仓库、Hacker News 热门文章及你手动保存的链接中筛选；如果首轮没有入选条目，再用 GitHub 公开仓库搜索补查可实战的全栈项目，最终选出**最多六条**写入私有 GitHub Issue。补查优先用中文搜索，核对 README 和近 7 天增星，再由 Jev 判断它与你的工作或学习是否相关；MiniMax 依据最终入选的公开原文，用白话解释“是什么、有什么用、第一步怎么试”。每条自动推荐都列出可点开的热度证据；两轮都没有合适内容就不凑数。
...
- Jev 模型固定 `jev-1.13.0`，首轮最多 20 次、仅在首轮 0 条时的项目补查最多额外 30 次，每自然日最多 50 次；每次请求正文最多 8 KiB。首次运行前创建当天占位 Issue，普通同日重跑不会重复收费；手动 `retry_empty` 会先标记 Issue，再补查一次。按 2026-09-24 [TypeSafe 公布的价格](https://docs.typesafe.ai/models) `$0.042/百万输入 token` 和原先的估算口径，31 天 Jev 理论上界约 `$0.55`；MiniMax 另计。**这不是 TypeSafe 账户级消费上限**：模型价格、服务商计费方式、用户删除日报后重跑等情况仍以控制台账单为准；如价格变化需暂停并重新核算。
...
`config.json` 的 `topics.work`、`topics.life` 关键词是 Jev 的个人相关性提示，不再限制热榜采集。旧 `github_queries`、`feeds` 字段仍可读取以兼容原有私有配置，但热度优先日报不再抓取或推荐这些来源。GitHub 读取今日/本周 Trending；Hacker News 读取热门榜前 30 条中近 48 小时、至少 30 分且带公开链接的文章。首轮去重后最多 20 条进入 Jev 判断；如果没有入选，再用 GitHub 仓库搜索优先查中文全栈项目，并以英文搜索补充。第二轮排除首轮已判断及近十四天已推荐的链接，最多检查 60 个仓库、最多额外判断 30 条。README 必须有前端、后端、数据库及运行或部署说明，且近 7 天有新增 star；创建 1 年内不设累计星数门槛，1–2 年需至少 1000 星，超过 2 年需至少 5000 星。通过者优先按近 7 天增星排序；这些条件只是公开线索，不保证项目可复现或质量。相关性门槛仍为工作或学习评分至少 `0.6`，不为了填满日报降低门槛。私有关注词可包含可实战的全栈项目，不要求使用 Go 或 AI Agent；模型只看公开标题和 README 摘录，不读取你工作的私有仓库。
